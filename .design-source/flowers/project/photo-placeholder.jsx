// Soft sepia-tinted placeholder block with paper grain + monospace caption.
// Used everywhere a real product photo will eventually go.
...
function PhotoPlaceholder({
  ...
  ratio = "4 / 5",
  caption,
  flourish = "petals", // petals | bouquet | sprig | none
  src = null,        // optional real image URL — when set, render <img> instead of artistic placeholder
      ...
      >
        <img
          ...
          style={{
            ...
            objectPosition,
            display: "block",
            // gentle warm sepia tint to match palette
            filter: "saturate(0.88) contrast(0.96) sepia(0.06)",
          }}
        />
        {/* subtle warm overlay to unify with the cream paper */}
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: `linear-gradient(180deg, rgba(244,236,216,0.04), rgba(60,40,30,0.10))`,
            pointerEvents: "none",
          }}
        />
        {/* caption corner */}
        ...
        >
          <span>{label}</span>
          {caption && <span style={{ textAlign: "right", maxWidth: "60%" }}>{caption}</span>}
        </div>
        <div
          style={{
            position: "absolute",
  ...
  return (
    ...
    >
      ...
      >
        ...
        <rect width="100%" height="100%" filter={`url(#grain-${gradId})`} />
      </svg>
      ...
      <div
        style={{
          position: "absolute",
          inset: 0,
          background: `radial-gradient(120% 100% at 50% 50%, transparent 55%, rgba(60,40,30,0.18) 100%)`,
          pointerEvents: "none",
        }}
      />
...
function Flourish({ kind, palette: p }) {
  ...
  if (kind === "petals") {
    return (
      <svg
        ...
        aria-hidden="true"
      >
        {/* big soft blob = bouquet silhouette */}
  ...
  if (kind === "bouquet") {
    return (
      ...
      >
        ...
        <path d="M50 78 L46 130 M52 78 L56 130 M48 80 L40 128 M53 80 L62 130"
              stroke={p.c} strokeWidth="1.2" opacity="0.5" fill="none" />
      </svg>
    );
  }
